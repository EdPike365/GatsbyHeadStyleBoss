...
import { getIIFEFunctionString, IIFEScriptComponent, injectIIFEComponentIntoTopOfBody} from "./ssr/IIFEFactory"
import { getStyleComponents, injectStylesIntoBottomOfHead } from "./ssr/StyleFactory"
...
export const onPreRenderHTML = ({
  ...
  injectStylesIntoBottomOfHead(styleComponents, getHeadComponents, replaceHeadComponents)
  ...
  injectIIFEComponentIntoTopOfBody( IIFEScriptComponent(iifeFunctionString), getPreBodyComponents, replacePreBodyComponents )  
...
const lazyInit = (pluginOptions) => {
  ...
  if(!pluginConfig){
    ...
    console.info("HSB: gatsby-ssr lazyInit(): pluginConfigs was null, so loading needed resources...")
    
    pluginConfig = pluginOptions.config  

    styleComponents = getStyleComponents(pluginConfig, fs)

    //browserIIFEScriptComponent = getBrowserIIFEFunctionScriptTag(pluginConfigs, fs)
    //IIFEScriptComponent = IIFEScriptComponent(pluginConfigs, fs)
