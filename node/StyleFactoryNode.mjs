...
export const processStyleResources = async pluginOptions => {
  const stylesCacheDir = getStylesCacheDir();
  if (!existsSync(stylesCacheDir)) {
    mkdirSync(stylesCacheDir, { recursive: true }, err => {
      console.error(err);
    });
  }
  ...
  pluginOptions.styleConfigs.forEach(styleConfig => {
    if (styleConfig.pathToCSSFile) {
      ...
      console.error(
        "No css file or remoteHREF was given for style with key = " +
          styleConfig.key
      );
      return;
    }
